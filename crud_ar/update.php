...
$valid = true;
if (empty($titre)) {
$titreError = 'Veuillez entrer un numero';
$valid = false;
}
 if(empty($contenu)) {
...
if ($valid) {
     ...
     $sql = "UPDATE article  set titre= ?, contenu=?, image=?,date_creation=? ,id_categorie=?,id_auteur=?,id_edition=? WHERE id_article = ?";
     $q = $pdo->prepare($sql);
     $q->execute(array($titre,$contenu,$phot,$date,$id_categorie,$id_auteur,$id_edition,$id));
     Database::disconnect();
     header("Location:../principale.php");}
